...
// #define ESP_BLE_MESH_VND_MODEL_OP_CARD_NUMBER ESP_BLE_MESH_MODEL_OP_3(0xC2, CID_ESP)
...
#define CARD_MAC_LENGTH   6
    ...
    esp_ble_mesh_msg_ctx_t ctx = {
        ...
        .app_idx = 0x0000,
        .addr = 0xffff,
        .send_ttl = MSG_SEND_TTL,   // TTL 值
        .send_rel = MSG_SEND_REL,
    };
    uint32_t opcode =ESP_BLE_MESH_VND_MODEL_OP_STATUS;
/**********************************自定义数据***************************** */
...
/* Function to send card number to client */

static const char* TRG = "rc522-demo";
static rc522_handle_t scanner;
SemaphoreHandle_t xBinarySemaphore;
SemaphoreHandle_t blue_public_state;
void assign_serial_number_to_array(uint64_t serial_number, uint8_t* array, size_t size);
/************************************************** */
...
static void uart_event_task(void *pvParameters)
{
    ...
    // char card[CARD_NUMBER_LENGTH]="0123456789";
    ...
    // 打印随机数 (调试用)
    printf("Generated dif_num: %u\n", card_msg.dif_num);

    // 发送消息
    esp_ble_mesh_server_model_send_msg(&vnd_models[0], &ctx, opcode, sizeof(card_msg), (uint8_t*)&card_msg);
     // 打印结果
    printf("Serial number as bytes: ");
...
static void rc522_handler(void* arg, esp_event_base_t base, int32_t event_id, void* event_data)
{
    ...
    switch(event_id) {
        case RC522_EVENT_TAG_SCANNED: {
                ...
                ESP_LOGI(TRG, "Tag scanned (sn: %" PRIu64 ")", tag->serial_number);
                // 将 serial_number 分解到数组
            assign_serial_number_to_array(tag->serial_number, card_msg.buffer, CARD_NUMBER_LENGTH);
...
void app_main(void)
{
    ...
    // 创建二值信号量
    blue_public_state = xSemaphoreCreateBinary();
    if (blue_public_state == NULL) {
        ESP_LOGE(TAG, "Failed to create binary semaphore!");
        return;
    }
    ESP_LOGI(TAG, "Binary semaphore created!");
...
void assign_serial_number_to_array(uint64_t serial_number, uint8_t* array, size_t size) {
    if (size < CARD_NUMBER_LENGTH) {
        printf("Error: Array size is too small.\n");
        return;
    }
    for (int i = 0; i < CARD_NUMBER_LENGTH; i++) {
        array[i] = (serial_number >> (8 * i)) & 0xFF; // 提取每个字节
    }
}
