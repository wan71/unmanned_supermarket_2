...
#define CORE_WIFI_BT   0 // 核心 0：运行 Wi-Fi 和 BLE Mesh 初始化
#define CORE_MQTT      1 // 核心 1：运行蓝牙和 MQTT 任务
...
typedef struct {
    ...
    uint8_t buffer[CARD_NUMBER_LENGTH];
    uint8_t dif_num; /* 随机数，防止重复 */
    uint8_t crc;
} card_msg;

typedef union {
    uint8_t raw_data[sizeof(card_msg)]; // 原始字节数组
    card_msg parsed_msg;               // 已解析的结构体
} card_msg_union;

// 定义队列句柄
QueueHandle_t messageQueue;

// card_msg_union received_data;
int smartconfig_success=0;
void task_MQTT(void *pvParameters);
void uint8ArrayToHexString(const uint8_t* input, size_t input_size, char* output, size_t output_size);
...
void process_received_data(const uint8_t *input_data);
/******************************************************************************* */
                                             ...
                                             esp_ble_mesh_model_cb_param_t *param)
{
    ...
    case ESP_BLE_MESH_CLIENT_MODEL_RECV_PUBLISH_MSG_EVT:
        // 处理接收到的数据
        process_received_data((uint8_t *)(param->client_recv_publish_msg.msg));   
        break;
...
//     ESP_ERROR_CHECK(err);
//     board_init();
//     // 创建队列，队列长度为10，单条消息大小为Message结构体的大小
//     messageQueue = xQueueCreate(10, sizeof(card_msg_union));
//     if (messageQueue == NULL) {
//         printf("Failed to create queue\n");
//         return;
//     }
//     initialise_wifi();
//     while(!smartconfig_success);
//     err = bluetooth_init();
//     if (err != ESP_OK) {
...
//     mqtt_app_start();
//     xTaskCreatePinnedToCore(task_MQTT, "task_MQTT", 4096, NULL, 4, NULL,1);
// }
...
void app_main(void)
{
    ...
    err = ble_mesh_nvs_open(&NVS_HANDLE);
    if (err) {
        ESP_LOGE(TAG, "Failed to open NVS for BLE Mesh");
        return;
    }
    // 获取设备 UUID 并初始化 BLE Mesh
    ble_mesh_get_dev_uuid(dev_uuid);
    err = ble_mesh_init();
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "Bluetooth mesh init failed (err %d)", err);
    ...
    ESP_LOGI(TAG, "Switched to Bluetooth priority");
    // 创建 Wi-Fi/Bluetooth 相关任务（绑定核心 0）
    // if (xTaskCreatePinnedToCore(task_ble_mesh, "task_ble_mesh", 4096, NULL, 4, NULL, CORE_WIFI_BT) != pdPASS) {
    //     ESP_LOGE(TAG, "Failed to create BLE Mesh task");
    //     return;
    // }

    // 创建 MQTT 任务（绑定核心 1）
    if (xTaskCreatePinnedToCore(task_MQTT, "task_MQTT", 4096, NULL, 3, NULL, CORE_MQTT) != pdPASS) {
...
{
    int msg_id;
    char mqtt_mac_str[CARD_MAC_LENGTH * 2 + 1];   // 用于存储 id 转换后的字符串
    char mqtt_buffer_str[CARD_NUMBER_LENGTH * 2 + 1]; // 用于存储 buffer 转换后的字符串
    const char* themes[] = {"id","mac"};
    const char* value_types[] = {"string","string"};
    const void* values[] = {mqtt_buffer_str,mqtt_mac_str};
    card_msg_union received_data;
    while (1)
    {
        // vTaskDelay(10000);
        if (xQueueReceive(messageQueue, &received_data, portMAX_DELAY) != pdPASS) {
            printf("Failed to receive message\n");
            }
        // 将 id 转换为 16进制字符串
        uint8ArrayToHexString(received_data.parsed_msg.id, CARD_MAC_LENGTH, mqtt_mac_str, sizeof(mqtt_mac_str));
        // 将 buffer 转换为 16进制字符串
        uint8ArrayToHexString(received_data.parsed_msg.buffer, CARD_NUMBER_LENGTH, mqtt_buffer_str, sizeof(mqtt_buffer_str));
        // 打印结果
        printf("mqtt_mac_str: %s\n", mqtt_mac_str);
        printf("mqtt_buffer_str: %s\n", mqtt_buffer_str);
...
void process_received_data(const uint8_t *input_data) {
    card_msg_union sended_data;
    /*计算大小*/
    int input_num=0;
    while((uint8_t)input_data[input_num]!=0xff)
    {
        input_num++;
        if(input_num >=255)
        {
            printf("Invalid data size: %zu,receive is fail\r\n", input_num);
            return;
        }
    }
    input_num++;
    ...
    // 将输入数据复制到 union 的 raw_data
    memcpy(sended_data.raw_data, input_data, input_num);

    // 打印解析后的结构体内容
    printf("Start: %u\n", sended_data.parsed_msg.start);
    printf("ID: ");
    for (int i = 0; i < CARD_MAC_LENGTH; i++) {
        printf("%02X ", sended_data.parsed_msg.id[i]);
    }
    printf("\nBuffer: ");
    for (int i = 0; i < CARD_NUMBER_LENGTH; i++) {
        printf("%02X ", sended_data.parsed_msg.buffer[i]);
    }
    printf("\nDif_Num: %u\n", sended_data.parsed_msg.dif_num);
    printf("CRC: %u\n", sended_data.parsed_msg.crc);

    // 将消息发送到队列
    xQueueSend(messageQueue, &sended_data, 0);
    
}

// 将 uint8_t 数组转换为 16进制字符串
void uint8ArrayToHexString(const uint8_t* input, size_t input_size, char* output, size_t output_size) {
    if (!input || !output || output_size < input_size * 2 + 1) {
        printf("Invalid parameters\n");
        return;
    }
    for (size_t i = 0; i < input_size; i++) {
        snprintf(output + i * 2, 3, "%02x", input[i]); // 每个字节生成2个字符
    }
    output[input_size * 2] = '\0'; // 添加字符串结束符
}
